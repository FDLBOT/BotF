...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'donasi':
            ...
            break
        case 'ownerbot':
            await aruga.sendContact(from, 628990542731)
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai, Im FDL BOT. To find out the commands on this bot type ${prefix}help`)
                      })
            } else {
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'Anjay Bahh', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'tts':
            ...
            const dataText = body.slice(8)
                if (dataText === '') return aruga.reply(from, 'text nya mana kak?..', id)
                try {
        ...
        case 'shortlink':
            ...
            break
		case 'bapakfont':
			if (args.length == 0) return aruga.reply(from, `Mengubah kalimat menjadi Fadhil Ganz\n\nketik ${prefix}bapakfont kalimat`, id)
        ...
        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot makanya Jangan Stress!', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *F D L B O T* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *F D L  B O T* 〙\n\n${msg}`)
            }
